...
#for i in Edges:
#                print(PowerIJ[i,t].X)     
OptimalResilienceShed = []
for t in Time:          
    OptimalResilienceShed.append(sum(Shed[i,t].X for i in Nodes))
...
#for i in Edges:
#                print(PowerIJ[i,t].X)              
HeuristicResilienceShed = []
for t in Time:          
    HeuristicResilienceShed.append(sum(Shed[i,t].X for i in Nodes))         
    print(sum((Shed[i,t].X for i in Nodes )))
...
NoResilienceShed = []
for t in Time:          
    NoResilienceShed.append(sum(Shed[i,t].X for i in Nodes))        
    print(sum((Shed[i,t].X for i in Nodes )))
#####
NumberNodesProtected = 2
NumberEdgesProtected = 4
PowerSub = nx.read_gml("Bus30WithData.gml")
PowerSub = nx.convert_node_labels_to_integers(PowerSub)
for i in PowerSub.nodes:
  for j in PowerSub.nodes:
...
#define workingness
for i in Nodes:
    model.addConstr(W_n[i,0] <= Grid.node[i]['working']+int(VectorProtectedNodes[i]))
    for t in range(1,len(Time)):
        model.addConstr(W_n[i,t]<=sum(F_n[i,g] for g in range(0,t))+int(Grid.node[i]['working']+int(VectorProtectedNodes[i])))
for e in Edges:
    for t in Time:
