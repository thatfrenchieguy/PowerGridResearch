# -*- coding: utf-8 -*-
"""
Created on Fri Sep  6 11:41:51 2019
...
for t in Time:
    for i in Nodes:
        for j in Nodes:
            ...
            model.addConstr(S[i,j,t] >= (1-X[i,j,t])*RepairCoefficient*RoadGrid[i][j]['weight']*speed - (1-K[i,j,t])*M)
...
TimeVaryingLengths = np.zeros((len(Nodes),len(Nodes),len(Time)))
for t in Time:
    for i in Nodes:
        for j in Nodes:
            if X[i,j,t].X == 1:
                TimeVaryingLengths[i][j][t] = RoadGrid[i][j]['weight']
            elif X[i,j,t].X == 0:
                TimeVaryingLengths[i][j][t] = RoadGrid[i][j]['weight']*RepairCoefficient
            else:
